...
class SiteController extends Controller
{
   ...
   public function detail_hotel($slug)
   {
    ...
    // print_r($category[0]['harga']);
    // echo "<br>";
    // print_r($category[1]['harga']);
    // die();
    ...
    $reservasi[] = Reservasi::where('category_kamar_id', $ct->id)->where('check_in','<=', Carbon::now())->where('check_out','>=', Carbon::now())->count();
       ...
       $harga_awal = $ct->harga;
        ...
        if ($ct->diskon) {
          ...
          if ($ct->diskon->diskon_start <= Carbon::now() && $ct->diskon->diskon_end >= Carbon::now()) 
            {

            if ($ct->diskon->type == "persen") {
              $harga = (int)$harga_awal - ((int)$harga_awal * (int)$ct->diskon/100);

            }elseif($ct->diskon->type == "potongan harga"){
              $harga =  (int)$harga_awal - (int)$ct->diskon->diskon;
            }
           }else {
                  $harga = $harga_awal;
              }
     ...
     dd($reservasi);
      return view('site.detail_hotel', compact('hotel', 'category','harga','harga_awal','icon','fasilitas', 'icon_hotel', 'fasilitas_hotel', 'reviews'));
   }
